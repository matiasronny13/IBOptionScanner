...
class ChainNormalizer:
    ...
    def breakChain(self, row):
        try:
            ...
            expires = [datetime.strptime(e, '%Y%m%d') for e in row['expirations'].split(' ')]

            # remove unnecessary strikes and expiration
            (expires, strikes) = self.filterContracts(row, strikes, expires)

            # combine strikes and expiration
            joinOutput = numpy.array(numpy.meshgrid(expires, strikes)).T.reshape(-1, 2)

            logger.info("ChainNormalizer {0} {1}: {2}".format(row['symbol'], row['optConId'], len(joinOutput)))

            for chain in joinOutput:
                obj = row.copy()
                del obj['expirations']
                del obj['strikes']
                obj['expiration'] = chain[0]
                obj['queryExpiration'] = chain[0].strftime('%Y%m%d')
