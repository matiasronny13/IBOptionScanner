...
class IBWrapper(wrapper.EWrapper):
    def __init__(self, db, limitCount, data):
        ...
        self.data = data
        self.totalRowCount = len(self.data)
        self.completedCount = 0

    def execute(self, rowIndex):
        pass

    def executeBatch(self, startIndex, endIndex):
        self.data[startIndex:endIndex].apply(self.execute, axis=1)
    ...
    def nextValidId(self, orderId: int):
        ...
        if len(self.data) > 0:
            logger.info('Processing {0} records ============================='.format(len(self.data)))

            self.data['rowId'] = range(0, len(self.data))
            chunkedIndexes = list(self.chunks(range(0, len(self.data)), self.limitCount))
            for index, rowIndexes in enumerate(chunkedIndexes):
                threading.Timer(index + 1, self.executeBatch, [rowIndexes[0], rowIndexes[-1]+1]).start()

    def onCompleted(self, reqId:int):
        self.completedCount += 1
        if self.completedCount >= self.totalRowCount:
            self.disconnect()
